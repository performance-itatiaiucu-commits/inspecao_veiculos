...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createItemEl(name, sectionName){
        ...
        ['Bom','Atenção','Defeito'].forEach(s=>{
            ...
            // Adicionar ícones aos botões de status
            let statusIcon = "✅";
            if (s === 'Atenção') statusIcon = "⚠️";
            if (s === 'Defeito') statusIcon = "❌";
        ...
        extra.innerHTML = `
            <textarea placeholder="✍️ Descreva o problema..."></textarea>
...
// Salvar inspeção no localStorage
...
function renderHistory(){
    ...
    out.innerHTML = '';
    if(!list.length){ 
        out.innerHTML = '<div style="color:var(--muted);padding:12px;text-align:center">📭 Nenhuma inspeção salva</div>'; 
        return; 
    }
    list.forEach(item=>{
        ...
        // Adicionar ícone ao histórico
        let statusIcon = "✅";
        const hasProblems = item.items.some(i => i.status !== 'Bom');
        if (hasProblems) statusIcon = "⚠️";
        ...
        card.innerHTML = `<div style="font-size:13px">
            ...
            <div style="font-size:12px;color:var(--muted)">${item.vehicleModel} • ${formatDateBR(item.date)}</div>
        </div>
        <div style="display:flex;gap:6px">
            <button class="secondary" data-id="${item.id}" aria-label="view" title="Ver inspeção">👁️</button>
            <button class="secondary" data-id="${item.id}" aria-label="pdf" title="Gerar PDF">📄</button>
            <button class="secondary" data-id="${item.id}" aria-label="del" title="Excluir">🗑️</button>
        </div>`;
        ...
        btnDel.addEventListener('click', ()=>{
            if(!confirm('🗑️ Remover esta inspeção?')) return;
...
function loadInspectionToForm(id){
    ...
    document.querySelectorAll('.item').forEach(it=>{
        ...
        if(found){
            ...
            // Atualizar visualmente o botão de status selecionado
